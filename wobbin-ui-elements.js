...
(function bootWobbinUiElements(){
  ...
  function scopedItems(){return typeof window.wobbinVisibleLibraryItems==='function'?window.wobbinVisibleLibraryItems():all()}
  function elementCount(key){return scopedItems().filter(x=>(S.platform==='All'||x.platform===S.platform)&&(x.elements||[]).includes(key)&&matchesQuery(x,S.query)).length}
  ...
  function filterMarkup(){
    ...
    const selectedCount=S.element==='All'?scopedItems().length:elementCount(S.element);
      ...
      <div class="wobbin-element-menu" data-element-menu>
        ...
        <div class="wobbin-element-scroll">
          <div class="wobbin-element-group">
            <button type="button" class="wobbin-element-option ${S.element==='All'?'selected':''}" data-element="All" data-element-search-text="all 全部"><span>All UI Elements</span><em>${scopedItems().length}</em></button>
          </div>
  ...
  home=function(){
    ...
    return `${header()}<main class="main"><div class="subnav"><div class="subnav-left">${platform()}<span class="divider"></span><span class="page-label"><strong>UI Elements</strong> · ${esc(S.element)}</span></div><button class="filter-btn">${filterIcon} Filter</button></div>${quickElements()}<div class="section-head"><div><h1>${esc(S.element)}</h1><p>${esc(zh)} · 所有匹配截图</p></div><span class="section-count">${list.length} screens</span></div>${list.length?screenView(list):`<div class="empty"><strong>没有对应截图</strong>调整 UI Element 或搜索条件。</div>`}</main>${suggestions()}`;
  };
  ...
  function ensureStyles(){
    ...
    style.textContent=`
      ...
      .wobbin-element-search input{width:100%;border:0;outline:0;background:transparent;color:var(--text,#fff);font:inherit;font-size:17px}.wobbin-element-search input::placeholder{color:var(--muted,#999)}
      .wobbin-element-scroll{max-height:calc(min(680px,72vh) - 60px);overflow:auto;padding:12px 12px 18px}.wobbin-element-group{display:grid;gap:3px;margin:0 0 12px}.wobbin-element-group h4{margin:8px 12px 7px;color:var(--muted,#aaa);font-size:12px;font-weight:700;text-transform:none}
      .wobbin-element-option{width:100%;height:46px;border:0;border-radius:13px;background:transparent;color:var(--text,#f7f7f7);display:flex;align-items:center;justify-content:space-between;padding:0 13px;font-size:15px;text-align:left;cursor:pointer}.wobbin-element-option:hover,.wobbin-element-option.selected{background:rgba(127,127,127,.18)}.wobbin-element-option>span{display:flex;align-items:center;gap:11px}.wobbin-element-option i{display:block;width:18px;height:18px;border-radius:5px;background:rgba(180,180,180,.32)}.wobbin-element-option.selected i{box-shadow:inset 0 0 0 5px currentColor}.wobbin-element-option em{font-style:normal;color:var(--muted,#999);font-size:12px}.wobbin-element-group.is-empty{display:none}
  ...
  function bindFilter(){
    const root=document.querySelector('[data-element-filter]');if(!root||root.dataset.bound==='1')return;root.dataset.bound='1';
    const toggle=root.querySelector('[data-element-filter-toggle]'),search=root.querySelector('[data-element-search]');
    ...
    search?.addEventListener('input',()=>{const q=String(search.value||'').trim().toLowerCase();root.querySelectorAll('.wobbin-element-option').forEach(btn=>{btn.style.display=!q||String(btn.dataset.elementSearchText||'').includes(q)?'':'none'});root.querySelectorAll('[data-element-group]').forEach(group=>{group.classList.toggle('is-empty',![...group.querySelectorAll('.wobbin-element-option')].some(b=>b.style.display!=='none'))})});
    if(!window.__WOBBIN_ELEMENT_OUTSIDE_BOUND){window.__WOBBIN_ELEMENT_OUTSIDE_BOUND=true;document.addEventListener('click',e=>{document.querySelectorAll('[data-element-filter].open').forEach(el=>{if(!el.contains(e.target))el.classList.remove('open')})})}
  }
