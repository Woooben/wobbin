...
(function bootWobbinPackageMeta(){
  ...
  function openEditor(){
    ...
    wrap.innerHTML=`<section class="modal wobbin-meta-modal"><header class="modal-head"><div><small>PACKAGE INFO</small><h2>编辑文件包信息</h2></div><button class="modal-close" type="button" data-app-meta-close>×</button></header><div class="wobbin-meta-form"><div class="wobbin-meta-logo-field"><span>App 图标</span><div class="wobbin-meta-logo-row">${logoPreview(meta)}<div class="wobbin-meta-logo-actions"><button type="button" class="secondary" data-app-logo-choose>选择图片</button><button type="button" class="secondary" data-app-logo-remove ${meta.logo_url?'':'disabled'}>移除图标</button><small>PNG / JPG / WebP，最大 5MB，建议使用正方形图片。</small></div><input hidden type="file" accept="image/png,image/jpeg,image/webp" data-app-logo-file></div></div><label><span>文件包名称</span><input type="text" maxlength="160" data-app-meta-name value="${attr(meta.name||'')}"></label><label><span>描述</span><textarea maxlength="800" rows="5" data-app-meta-description placeholder="例如：旅行住宿预订产品，包含搜索、房源详情、预订与消息等页面。">${esc(meta.description||'')}</textarea><em data-app-meta-count>${String(meta.description||'').length}/800</em></label><div class="wobbin-meta-type-field"><span>产品类型 <small>可多选</small></span>${typePicker()}</div></div><footer class="modal-foot"><button type="button" class="secondary" data-app-meta-cancel>取消</button><button type="button" class="primary" data-app-meta-save>保存</button></footer></section>`;
