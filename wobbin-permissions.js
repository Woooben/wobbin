...
async function publicCloudFetch(payload,{form=false}={}){
  const headers={};
  let body;
  if(form)body=payload;
  else{headers['Content-Type']='application/json';body=JSON.stringify(payload||{})}
  const res=await fetch(WOBBIN_ADMIN_URL,{method:'POST',headers,body});
  let data={};try{data=await res.json()}catch{}
  if(!res.ok)throw new Error(data.error||`云端操作失败（${res.status}）`);
  return data;
}

/* Route upload-only actions through the public endpoint. */
adminFetch=async function(payload,opts={}){
  ...
  /* Failed direct PUTs normally leave no object. Do not ask visitors for an admin key just to clean a failed path. */
...
/* Upload capability check is public; it must never prompt visitors for the admin key. */
validateAdmin=async function(){return publicCloudFetch({action:'upload-capabilities'})};

async function checkAdminKeySilently(key){
  if(!key)return false;
  try{
    const res=await fetch(WOBBIN_ADMIN_URL,{
      method:'POST',
...
async function initWobbinPermissions(){
  let saved='';
  try{saved=localStorage.getItem(WOBBIN_ADMIN_KEY_STORE)||''}catch{}
  const params=new URLSearchParams(location.search);
  const wantsAdmin=params.get('admin')==='1';

  if(wantsAdmin){
    const entered=prompt('请输入 Wobbin 管理员口令');
    clearAdminQuery();
    if(entered&&await checkAdminKeySilently(entered.trim())){
      try{localStorage.setItem(WOBBIN_ADMIN_KEY_STORE,entered.trim())}catch{}
    ...
    WOBBIN_PERMISSIONS.admin=await checkAdminKeySilently(saved);
    if(!WOBBIN_PERMISSIONS.admin){try{localStorage.removeItem(WOBBIN_ADMIN_KEY_STORE)}catch{}}
  }

  WOBBIN_PERMISSIONS.checked=true;
  render();
}
...
function applyWobbinPermissionUI(){
  ...
  if(S.batchMode){S.batchMode=false;S.batchSelected?.clear?.()}
  if(S.coverPicker)S.coverPicker=null;

  const selectors=[
    '[data-cover-app]',
    '#set-cover-current',
    '[data-delete-app]',
    '[data-delete-current-app]',
    '[data-delete-screen]',
    '[data-preview-delete]',
    '[data-batch-toggle]',
    '[data-batch-delete]',
  ...
  document.querySelectorAll('.card-actions').forEach(el=>{if(!el.children.length)el.remove()});
  document.getElementById('cover-bg')?.remove();
}

const __wobbinPermissionsRender=render;
render=function(){
  __wobbinPermissionsRender();
  queueMicrotask(()=>queueMicrotask(applyWobbinPermissionUI));
};
