...
(function bootWobbinMobbinDetail(){
  ...
  function cloudAppRow(){
    try{
      if(window.WOBBIN_CLOUD?.apps instanceof Map){
        for(const row of WOBBIN_CLOUD.apps.values())if(row?.name===S.app)return row;
      }
    }catch{}
    return null;
  }
  function appTypes(items=appItems()){
    const fromItems=items.flatMap(x=>Array.isArray(x.appTypes)?x.appTypes:[]);
    const row=cloudAppRow();
    return uniq([...fromItems,...(Array.isArray(row?.app_types)?row.app_types:[])]);
  }
  ...
  function ratingId(){return String(cloudAppRow()?.id||S.app||'')}
  ...
  function saveRating(value){const map=ratings();map[ratingId()]=Number(value);localStorage.setItem(RATING_KEY,JSON.stringify(map))}
  ...
  function productTags(types){return types.length?`<div class="mobbin-direct-tags">${types.map(x=>`<span>${esc(x)}</span>`).join('')}</div>`:''}
  ...
  function detailTabs(items,list){
    const resultLabel=S.tab==='elements'?'UI elements':S.tab==='flows'?'flow screens':'screens';
    ...
    return `<main class="main wobbin-mobbin-detail">
      <section class="mobbin-detail-hero">
        ...
        <div class="mobbin-product-meta">
          ...
          <div class="mobbin-meta-block"><span>Category</span><strong>${esc(types.join(', ')||'—')}</strong></div>
        </div>
      </section>
  ...
  bind=function(){
    ...
    document.querySelectorAll('[data-tab]').forEach(btn=>{
      btn.onclick=()=>{S.tab=btn.dataset.tab;S.platform='All';S.element='All';S.flowFilter='All';render()};
    });
    ...
    more?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const open=popover?.hasAttribute('hidden');if(open)popover?.removeAttribute('hidden');else popover?.setAttribute('hidden','');more.setAttribute('aria-expanded',open?'true':'false')});
    popover?.addEventListener('click',e=>e.stopPropagation());
  };

  if(typeof selectedVisibleScreenIds==='function')selectedVisibleScreenIds=function(){return detailItems().map(x=>x.id)};
  ...
  function ensureStyles(){
    ...
    const style=document.createElement('style');style.id='wobbin-mobbin-detail-style';style.textContent=`
      ...
      .mobbin-product-meta{display:grid;grid-template-columns:110px minmax(210px,auto) minmax(200px,1fr);gap:30px;margin-top:34px;margin-left:124px}.mobbin-meta-block{min-width:0}.mobbin-meta-block>span{display:block;margin-bottom:7px;color:var(--muted,#8f8f8f);font-size:10px;font-weight:600}.mobbin-meta-block>strong{display:block;max-width:520px;color:var(--text,#fff);font-size:14px;line-height:1.35;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
      .mobbin-rating-control{display:flex;align-items:center;gap:1px;min-height:20px}.mobbin-rating-control button{width:18px;height:22px;padding:0;border:0;background:none;color:#555;font-size:15px;line-height:1;cursor:pointer}.mobbin-rating-control button:hover,.mobbin-rating-control button.active{color:var(--text,#fff)}.mobbin-rating-control span{margin-left:7px;color:var(--text,#fff);font-size:13px;font-weight:700}
      ...
      .mobbin-batch-anchor{display:none!important}.wobbin-mobbin-detail .batch-toolbar{margin:0 0 24px!important}.wobbin-mobbin-detail .screens-grid{grid-template-columns:repeat(6,minmax(0,1fr));gap:28px 18px}.wobbin-mobbin-detail .screen-frame{min-height:0!important;padding:0!important;border-radius:22px!important;background:transparent!important;align-items:flex-start!important;overflow:visible!important}.wobbin-mobbin-detail .screen-frame img{width:100%!important;max-width:100%!important;height:auto!important;object-fit:contain!important;border-radius:22px!important;box-shadow:none!important;background:var(--panel2,#222)}.wobbin-mobbin-detail .screen-name{margin-top:9px}.wobbin-mobbin-detail .screen-name strong{font-size:11px}.wobbin-mobbin-detail .screen-name span{font-size:9px}.wobbin-mobbin-detail .section-head{margin:0 0 14px}.wobbin-mobbin-detail .section-head h1{font-size:15px!important}.wobbin-mobbin-detail .section-head p{font-size:10px}
      ...
      @media(max-width:1180px){.wobbin-mobbin-detail{width:min(calc(100% - 44px),1120px)!important}.wobbin-mobbin-detail .screens-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}
      @media(max-width:900px){.wobbin-mobbin-detail{width:calc(100% - 32px)!important;padding-top:24px!important}.wobbin-mobbin-detail .app-title{grid-template-columns:72px minmax(0,1fr);gap:18px}.wobbin-mobbin-detail .app-title>.app-icon{width:72px;height:72px;border-radius:17px}.wobbin-mobbin-detail .app-title h1{margin-top:10px;font-size:34px}.mobbin-product-meta{margin-left:90px;grid-template-columns:1fr 1fr;gap:22px}.mobbin-meta-block:last-child{grid-column:1/-1}.wobbin-mobbin-detail .screens-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
      @media(max-width:640px){.wobbin-mobbin-detail{width:calc(100% - 24px)!important}.mobbin-detail-hero{min-height:0;padding-bottom:34px}.wobbin-mobbin-detail .app-title{grid-template-columns:58px minmax(0,1fr);gap:14px}.wobbin-mobbin-detail .app-title>.app-icon{width:58px;height:58px;border-radius:14px}.wobbin-mobbin-detail .app-title h1{margin-top:4px;font-size:27px}.mobbin-detail-menu{margin-top:4px}.mobbin-product-meta{margin-left:0;grid-template-columns:1fr;gap:18px;margin-top:28px}.mobbin-meta-block:last-child{grid-column:auto}.mobbin-detail-tabs{display:block}.mobbin-result-count{display:block;margin-top:12px;text-align:left}.wobbin-mobbin-detail .screens-grid{grid-template-columns:repeat(2,minmax(0,1fr));gap:22px 10px}.mobbin-detail-tabs-left{gap:12px}.mobbin-tab-divider{display:none}}
